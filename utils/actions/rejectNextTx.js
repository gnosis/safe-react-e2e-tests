import { assertElementPresent, assertTextPresent, clickByText, clickElement, getInnerText } from '../selectorsHelpers'
...
export const rejectNextTx = async (gnosisPage, metamask) => {
  ...
  await assertTextPresent(
    { selector: '#infinite-scroll-container > div > p', type: 'css' },
    'NEXT TRANSACTION',
    gnosisPage,
  )
  ...
  await gnosisPage.waitForFunction(
    (nonce) => document.querySelector('div.tx-nonce').innerText.includes(nonce),
    {},
    currentNonce,
  )
