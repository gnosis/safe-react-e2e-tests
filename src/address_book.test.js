...
beforeAll(async () => {
  ;[browser, metamask, gnosisPage] = await initWithDefaultSafe(true)
}, 60000)
...
describe("Address book", () => {
  ...
  const filePath = path.relative(process.cwd(), __dirname + '/../utils/files/address_book_test.csv')
  const ENSName = "francotest.eth"
    ...
    try{
        ...
        await isTextPresent(addressBook.entryModal.selector, "Must be a valid address, ENS or Unstoppable domain", gnosisPage)
        ...
        await gnosisPage.evaluate( (editName, rowSelector, editEntryBtn) =>{ 
        ...
        }, accountsSelectors.otherAccountNames.owner5_name, addressBook.entryRowSelector.selector, addressBook.editEntryBtn.selector)
        ...
        await gnosisPage.evaluate( (editName, rowSelector, deleteEntryBtn) =>{ 
          const rows = Array.from(document.querySelectorAll(rowSelector))
          const index = rows.findIndex( e => e.querySelector("td").innerText.includes(editName))
          rows[index].querySelector(deleteEntryBtn).click()
        }, editedName, addressBook.entryRowSelector.selector, addressBook.deleteEntryBtn.selector)
        await clickElement(addressBook.deleteSubmitBtn, gnosisPage)
        const OwnerRemovedIndex = await gnosisPage.evaluate( (rowSelector, editedName) => {
          const rows = Array.from(document.querySelectorAll(rowSelector))
          const index = rows.findIndex( e => e.innerText.includes(editedName))
          return index
        }, addressBook.entryRowSelector.selector, editedName)
        await gnosisPage.waitForTimeout(2000)
